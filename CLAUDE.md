...
- 테스트: `.venv/Scripts/python.exe -m pytest -q`  ✅ 동작 (현재 73 passed)
...
- 현재 상태: **MVP 전체 구현 완료** (엔진 + 정규화 + 데이터 레이어 + 오케스트레이터 + LLM augmenter + FastAPI/UI, 73 tests). 다음 후보: 모델 보정(과거경기 피팅)·백테스트, API-Football 통합, 토너먼트 시뮬레이션.
