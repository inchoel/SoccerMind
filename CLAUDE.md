...
- 테스트: `.venv/Scripts/python.exe -m pytest -q`  ✅ 동작 (현재 100 passed)
...
- 현재 상태: **MVP + 보정/백테스트(metrics/calibration) + 토너먼트 DP(tournament, `/api/tournament`) 완료, 100 tests.** 다음 후보: 과거경기 데이터 수집(보정 입력), API-Football 통합, 토너먼트 UI.
