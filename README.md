...
| 📈 **최근 폼·실제 맞대결** | Wikipedia 결과에서 팀별 최근 W/D/L + 이미 치러진 경기 결과 감지 |
| 🧭 **분석 코멘트** | 전력 평가(eloratings) + 특이사항·리스크·관전 포인트 (데이터 기반, 가짜 인용 없음) |
| 📰 **전문가 프리뷰** | 고객 선택 시 실제 웹검색으로 경기 프리뷰를 출처와 함께 표시 (`previews=1`) |
